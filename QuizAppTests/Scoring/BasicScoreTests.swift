...
class BasicScoreTests: XCTestCase {
    ...
    func test_MatchingAnswerOneNonMatching_scoresOne() {
    ...
    func test_withTooManyCorrectAnswers_oneMatchingAnswer_scoresOne() {
        let score = BasicScore.score(for: ["not a match", "another answer"], comparingTo: ["an answer", "another answer", "an extra answer"])
        XCTAssertEqual(score, 1)
    }
}
